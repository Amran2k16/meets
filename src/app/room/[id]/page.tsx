...
import VideoPresentation, { Video } from "./_components/video-presentation";
...
import { cn, emitAsync, safeEmitAsync } from "@/lib/utils";

const createVideoEntry = (id: number, title: string, producerId: string, stream: MediaStream): Video => ({
  id,
  title,
  thumbnail: "bg-muted/50",
...
export default function Page() {
  ...
  const [presentingVideoId, setPresentingVideoId] = useState<number | null>(null);
  ...
  // ================================================
  const handleNewProducer = async ({ socketId, producerId }: { socketId: string; producerId: string }) => {
    ...
    const stream = new MediaStream([consumer.track]);
    console.log(`Stream ID: ${stream.id}, Producer ID: ${producerId}`);
    setRemoteStreams((prev) => ({ ...prev, [producerId]: stream }));
  ...
  const togglePresentationMode = () => {
    ...
    if (!isPresentationMode) {
      setPresentingVideoId(videos[0]?.id || null);
    } else {
  ...
  return (
    <div className="flex flex-col px-2 md:px-4 pt-2 md:pt-4" style={{ minHeight: PAGE_LAYOUT_HEIGHT }}>
      {/* Toggle Button */}
      <div className="mb-4 flex justify-end">
        <button onClick={togglePresentationMode} className="px-4 py-2 bg-primary text-white rounded-lg shadow">
      ...
      <div className="grid auto-rows-min gap-4 md:grid-cols-4">
        {localStream && (
          <div className={cn("aspect-video rounded-xl cursor-pointer", "bg-muted/50")}>
            <video
              ref={(el) => {
                if (el) {
              ...
              muted
            />
            <span className="block text-center text-muted-foreground">Local Stream</span>
          </div>
        )}

        {Object.entries(remoteStreams).map(([producerId, stream]) => (
          <div key={producerId} className={cn("aspect-video rounded-xl cursor-pointer", "bg-muted/50")}>
            <video
              ref={(el) => {
                if (el) {
                  el.srcObject = stream;
